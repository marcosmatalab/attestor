...
enacted** with the Regulation **as it binds today**. Knowing what changed, and
when, is part of the answer a compliance owner needs.

That the module survived the Omnibus becoming law with a constant change and no
new logic is the point of the design: the caveat is read from the bundle's own
``meta.status_note`` — a single source of truth — and never hardcoded here,
because the status is exactly the thing that changes.
"""
...
class TimelineComparison(BaseModel):
    ...
    # Status note of the bundle that BINDS, read from its ``meta``. The field keeps
    # the name ``omnibus_status`` so the API and the frontend do not break; what it
    # carries is now the in-force note rather than the provisional one.
    omnibus_status: str
...
def compare_timelines(
    ...
    *,
    ...
    The second bundle defaults to the one in force. Pass ``omnibus_bundle`` explicitly
    to compare against the frozen provisional overlay instead.
    """
