...
class AnnexIVDossier(BaseModel):
    ...
    # ``meta`` and never composed here. It was called ``provisional_note`` while the only
    # non-empty value was a provisional caveat; once the Omnibus became law that name
    # would have made a dossier state the opposite of the truth.
    status_note: str
