...
def _pin(tmp_path: Path) -> list[str]:
    """``--public-key`` pointing at the test key, the way an auditor would run it."""
    key_file = tmp_path / "operator.hex"
    key_file.write_text(public_key_hex(_TEST_KEY.public_key()), encoding="utf-8")
    return ["--public-key", str(key_file)]


def test_cli_accepts_a_valid_ledger(tmp_path: Path) -> None:
    save_ledger(tmp_path, _RECORDS, seal(_RECORDS, _TEST_KEY))
    assert main([str(tmp_path), *_pin(tmp_path)]) == 0
