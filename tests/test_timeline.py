...
def test_caveat_is_read_from_the_bundle_meta_not_hardcoded() -> None:
    """The status text must come from the bundle that binds, whichever that is.

    This test is the reason the Omnibus becoming law cost one constant: the caveat
    was never a literal in the code, so the value changed and the mechanism did not.
    """
