...
def test_no_omnibus_date_leaked_into_legal_text_bundle() -> None:
    # The 2026-12-02 legacy-marking transition is an Omnibus delta, not legal text.
    bundle = load_bundle("v2026-08")
...
def test_historical_omnibus_bundle_keeps_its_provisional_status() -> None:
    """It described a real situation on 2026-06-23 and must keep describing it.

    Editing this bundle to say the Omnibus is now law would change its sha256 and
    every checksum derived from it, and would destroy the comparison that makes
    the regulatory-evolution story checkable.
    """
    ...
    assert "provisional" in bundle.meta["status_note"].lower()


def test_in_force_bundle_states_when_it_started_binding() -> None:
    bundle = load_bundle("reg-2026-1744")
    assert bundle.meta["status"] == "in-force"
    assert bundle.meta["in_force_since"] == "2026-07-27"
