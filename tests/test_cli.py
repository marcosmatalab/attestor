...
def test_checksum_only_prints_exactly_the_checksum(capsys: pytest.CaptureFixture[str]) -> None:
    code = main(
...
def test_checksum_is_the_same_on_a_second_run(capsys: pytest.CaptureFixture[str]) -> None:
...
def test_bundle_flag_selects_the_scenario(
    bundle: str, expected: str, capsys: pytest.CaptureFixture[str]
) -> None:
...
def test_full_classify_output_lists_obligations_with_dates(
    ...
    assert "Art. 9" in out
    assert "2027-12-02" in out  # the deferred Annex III date, from the bundle in force


def test_unknown_bundle_exits_with_the_usage_code(capsys: pytest.CaptureFixture[str]) -> None:
    assert main(["classify", "--role", "provider", "--bundle", "nope", "--checksum-only"]) == 2
    assert "could not load bundle" in capsys.readouterr().err


def test_invalid_profile_combination_exits_with_the_usage_code(
    capsys: pytest.CaptureFixture[str],
) -> None:
    # generates_synthetic_content requires content_lifecycle.
    assert main(["classify", "--role", "provider", "--generates-synthetic-content"]) == 2
...
def test_ledger_verify_reports_tampering_with_exit_1(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    source = Path(EXAMPLE_LEDGER)
    edited = (source / "records.json").read_text(encoding="utf-8").replace("sys-1", "sys-9")
    (tmp_path / "records.json").write_text(edited, encoding="utf-8")
    ...
    assert "signature_ok = True" in out  # the root was not touched


def test_missing_ledger_is_a_usage_error_not_a_pass(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
...
def test_demo_prints_both_provenance_axes_and_verifies(
    capsys: pytest.CaptureFixture[str],
) -> None:
    assert main(["demo"]) == 0
    out = capsys.readouterr().out
